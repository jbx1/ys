...
public class AdminApiImpl implements AdminApi {
    ...
    @Override
    public ResponseEntity<AccreditationResponse> createAccreditation(CreateAccreditationRequest createAccreditationRequest) {

        logger.info("Received: {}", createAccreditationRequest);
        try {
            AccreditationResponse response = adminService.createAccreditation(createAccreditationRequest);
            return ResponseEntity.ok(response);
        } catch (Exception ex) {
            logger.error("Unable to perform operation", ex);
            //let's avoid leaking internal errors to the front-end
            throw new UnableToPerformOperationException("Unable to perform the requested operation. Please refer to the logs for more details.");
        }
    }

    @Override
    public ResponseEntity<AccreditationResponse> finalizeAccreditation(UUID accreditationId, FinaliseAccreditationRequest finaliseAccreditationRequest) {

        try {
            return adminService.finalizeAccreditation(accreditationId, finaliseAccreditationRequest.getOutcome())
